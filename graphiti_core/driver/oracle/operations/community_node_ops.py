...
from graphiti_core.driver.oracle.rdf_utils import (
    ...
    fetch_community_node_embedding,
    get_embedding_table_name,
    parse_float_list_literal,
    rdf_mode_for_executor,
    sparql_string_literal,
    upsert_community_node_embedding,
    upsert_community_node_embeddings_bulk,
)
...
def _normalize_community_record(record: Any) -> dict[str, Any]:
...
COMMUNITY_NODE_JOIN_SELECT_COLUMNS = [
    'm.uuid AS uuid',
    'm.name AS name',
...
COMMUNITY_NODE_SPARQL_CORE_PATTERNS = """
...
class OracleCommunityNodeOperations(CommunityNodeOperations):
    ...
    async def save(
        ...
        subject = build_node_subject('community', node.uuid)
        ...
        await upsert_community_node_embedding(
            executor,
            node.uuid,
            node.name_embedding,
            node.summary,
    ...
    async def save_bulk(
        ...
        updates = [
            build_subject_upsert_update(
                build_node_subject('community', node.uuid),
                {
                    'type': 'Community',
                    'uuid': node.uuid,
                    'name': node.name,
        ...
        await upsert_community_node_embeddings_bulk(
            executor,
    ...
    async def delete(
        ...
        logger.debug(f'Deleted Community Node from RDF Graph: {node.uuid}')
    ...
    async def delete_by_uuids(
        ...
        embedding_table = get_embedding_table_name(executor, 'COMMUNITY_NODES')
        subjects = [build_node_subject('community', node_uuid) for node_uuid in uuids]
        if subjects:
            await execute_sparql_update(executor, build_delete_subjects_update(subjects), tx=tx)
        if uuids:
            await delete_embeddings_bulk(executor, embedding_table, uuids, tx=tx)
    ...
    async def get_by_uuid(
        ...
        LIMIT 1
        """
        records = await self._query_nodes_with_embeddings(
            executor,
            sparql_query,
            limit=1,
        )
    ...
    async def get_by_uuids(
        ...
        """
        return await self._query_nodes_with_embeddings(executor, sparql_query)
    ...
    async def load_name_embedding(
        ...
        node.name_embedding = await fetch_community_node_embedding(executor, node.uuid)
