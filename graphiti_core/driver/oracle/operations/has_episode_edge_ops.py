...
class OracleHasEpisodeEdgeOperations(Neo4jHasEpisodeEdgeOperations):
    ...
    async def save(self, executor, edge, tx=None) -> None:
        if rdf_mode_for_executor(executor):
            subject = build_edge_subject('has_episode', edge.uuid)
            ...
            logger.debug(f'Saved HAS_EPISODE edge to RDF Graph: {edge.uuid}')
            return
    ...
    async def save_bulk(self, executor, edges, tx=None, batch_size: int = 100) -> None:
        if rdf_mode_for_executor(executor):
            updates = [
                build_subject_upsert_update(
                    build_edge_subject('has_episode', edge.uuid),
                    {
                        'type': 'HAS_EPISODE',
                        'uuid': edge.uuid,
                        'group_id': edge.group_id,
    ...
    async def delete(self, executor, edge, tx=None) -> None:
        if rdf_mode_for_executor(executor):
            await execute_sparql_update(
                executor,
                build_delete_subjects_update([build_edge_subject('has_episode', edge.uuid)]),
                tx=tx,
            )
            logger.debug(f'Deleted HAS_EPISODE edge from RDF Graph: {edge.uuid}')
            return
        await super().delete(executor, edge, tx=tx)

    async def delete_by_uuids(self, executor, uuids, tx=None) -> None:
        if rdf_mode_for_executor(executor):
            subjects = [build_edge_subject('has_episode', edge_uuid) for edge_uuid in uuids]
            if subjects:
