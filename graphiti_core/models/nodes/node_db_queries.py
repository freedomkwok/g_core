...
from graphiti_core.driver.oracle.rdf_utils import build_node_subject, build_subject_upsert_update
        ...
        case GraphProvider.ORACLE:
            ...
            for node in nodes:
                ...
                if isinstance(attributes_value, dict):
                    ...
                    # Preserve custom properties under a single RDF attributes object.
                    attributes = {
                        key: value
                ...
                updates.append(
                    build_subject_upsert_update(
                        build_node_subject('entity', str(node.get('uuid', ''))),
                        {
                            'name': node.get('name'),
            ...
            return '; '.join(update for update in updates if update)
        case GraphProvider.NEPTUNE:
            queries = []
            for node in nodes:
                labels = ''
